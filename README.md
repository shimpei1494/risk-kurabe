...
パッケージマネージャーは **`pnpm@11.5.2`** を使用。依存関係の管理は `vp install` などの Vite+ コマンドで行い、`pnpm` / `npm` / `yarn` を直接呼ぶのは避けてください。
...
環境の確認には `vp env current` が便利です。`.node-version` から解決されたNode.jsのバージョンと、`packageManager` で指定されたパッケージマネージャーのバージョンを表示できます。
...
- `vp run fallow` — 未使用ファイル・依存関係・エクスポートの検出（`.fallowrc.json`）
