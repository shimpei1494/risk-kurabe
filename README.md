...
3. 比較地点の追加時に「地図から選ぶ」へ切り替え、クリックしたピン座標から地点を追加できる
4. 結果に地点の表示住所が表示され、緯度経度は表示されない
5. 関東内の地点でR2から調査結果が表示される
6. 東京都内では地震時の総合危険度と、その内訳である建物倒壊・火災危険度が表示される
7. 地図を最大浸水、地震総合、建物倒壊、火災で切り替えられる
8. 地図をクリックすると浸水深を確認できる
9. 結果地図のピンを2km以内で移動し、確認後に同じ地点の結果を更新できる
10. 関東外では各指標が「対象外」と表示される
11. 境界付近で「付近で判定が変わります」と表示される
