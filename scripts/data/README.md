...
データセットごとの取得元、収録範囲、加工内容および既知の制約は
[`docs/データ/公開GISデータの来歴と生成.md`](../../docs/データ/公開GISデータの来歴と生成.md)
を参照する。
...
地盤分類、建物倒壊危険度、火災危険度、災害時活動困難係数および総合危険度を
正規化する。危険度3種は公式の危険量（棟/ha）、東京都内順位、ランク1〜5を保持する。
Shapefileの`ID`は公式町丁目コードではなく連番のため`source_id`として保持し、
名称組を`town_key`とする。
