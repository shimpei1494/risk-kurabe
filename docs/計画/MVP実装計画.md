...
- 東京都内の「東京都・地震時の総合危険度」ランク
- 総合危険度の内訳・根拠となる建物倒壊危険度、火災危険度、災害時活動困難係数および地盤分類
...
- A31a、東京都地域危険度の結果表示
- 東京都地域危険度は、地震時の総合危険度を主表示し、建物倒壊・火災・災害時活動困難係数・地盤分類を詳細表示
...
- 東京都・地震時の総合危険度を主表示し、建物倒壊・火災・災害時活動困難係数・地盤分類を詳細表示
...
- 重視条件、アプリ独自の災害横断総合スコア、地点の推奨および順位付け
- 期限付き・取り消し可能な共有リンク
...
初期の分割単位は都県・データ種別とする。Day 2の性能計測で必要な場合だけ水系単位へ細分化する。
...
5. 東京都内だけ地域危険度町丁目を検索
6. 建物倒壊、火災および東京都・地震時の総合危険度について、25m bboxの候補を取得し、25mバッファ内の異なる判定を確認
7. データ状態、境界警告、根拠および版を含む`InvestigationResult`を返す
...
- 東京都の総合危険度を洪水等を含む災害横断スコアとして扱わない
- 地図レイヤー変更で判定を再実行しない
...
- 1地点では縦方向の詳細カード
- 2〜3地点では同じ指標を列で比較
- 洪水と東京都の地震地域危険度を別セクションとして表示する
- 東京都の地震地域危険度では、`東京都・地震時の総合危険度`を折りたたまない主表示とする
- `東京都内の町丁目間の相対評価であり、洪水などは含まない`ことを総合危険度の近くに常時表示する
- 建物倒壊危険度、火災危険度、災害時活動困難係数および地盤分類は、総合危険度の内訳・根拠として詳細開閉から確認できるようにする
- 地盤分類は町丁目単位の分類であり、個別敷地の地盤調査や液状化判定ではないことを明記する
- 東京都外は総合危険度とその内訳を`対象外（東京都のみの指標）`とし、最低ランクへ変換しない
- データ状態は値と同じ位置に表示し、色だけに依存しない
- 境界警告は該当指標の近くに表示
- 根拠情報は詳細開閉で確認可能
- 結果表の指標選択と地図レイヤーを連動
- 総合危険度、建物倒壊危険度、火災危険度を地図で切り替え可能にする
- 地図には1主題レイヤーと全地点ピンを表示
- ピン地点の階級を常時表示し、周辺はホバーまたはタップで確認可能にする
- 結果地図のピンは2km以内で移動可能とし、確認操作後に正式な地点判定と比較結果を更新する
...
- 重複河川・水系の全根拠保持
- 東京都地域危険度の対象内・対象外
- 東京都・地震時の総合危険度のランク、都内相対評価および対象外
- 総合危険度、建物倒壊危険度、火災危険度それぞれの境界警告
- 地盤分類と災害時活動困難係数の詳細表示用変換
...
- 東京都地域危険度FGBの総合危険度、建物倒壊、火災、災害時活動困難係数および地盤分類が有効である

### 結合・手動E2E

最低限、次のデモ経路を毎日確認する。

1. 東京都内1地点: 洪水 + 東京都・地震時の総合危険度 + 内訳詳細
2. 東京と県外の2地点: 洪水比較 + 東京都の地震地域危険度の対象外
3. 3地点: 複数河川が重なる地点を含む比較
...
- GIS加工ツールの導入方法と版を固定
- A31a、東京都地域危険度の入力ファイルとチェックサムを固定
...
- 型と純粋関数を実装
- A31a、東京都地域危険度を`InvestigationResult`へ統合
- データ状態、全根拠、境界警告を実装
- 単体テストとデータ契約テストを完成
...
- localStorageへの保存・復元・個別削除・全削除
- 上限10件と90日の失効

#### 第2段階: 東京都の地震地域危険度表示を再構成

- 既存の東京都地域危険度FGBに含まれる`overall_score`、`overall_order`、`overall_rank`、`activity_difficulty`および`ground_classification`を利用する
- `InvestigationResult`へ東京都・地震時の総合危険度とその境界警告を追加し、判定ロジック版とsessionStorageスキーマ版を更新する
- 結果カードでは、東京都・地震時の総合危険度を地震セクションの主表示にする
- 建物倒壊危険度、火災危険度、災害時活動困難係数および地盤分類を、展開可能な内訳・根拠として表示する
- 比較画面では総合危険度を常時表示し、内訳は必要なときだけ展開する
- `洪水などを含む総合値ではない`、`東京都内の相対評価`、`個別敷地の地盤調査ではない`という説明を画面と根拠情報に固定する
- 既存FGBの`overall_rank`から総合危険度専用PMTilesを追加し、建物倒壊・火災と切り替え可能にする
- 1地点、東京都内2〜3地点、東京都と県外の混在、町丁目境界、キャッシュ旧版破棄をテストする

完了条件: 東京都内では地震時の総合危険度が最初に理解でき、内訳を確認できる。東京都外との混在時にも、洪水を含む総合評価や県外の低リスクを示す表示にならない。

#### 第3段階: 地図上の値確認と近隣地点の再判定（完了）

- ピン位置に表示中指標の階級ラベルを常時表示する
- PCのホバーとタッチ端末のタップで、周辺位置の階級を表示する
- ピン移動後に確認操作を挟み、既存のFlatGeobuf地点判定で結果を更新する
- 1〜3地点の同じ地点枠を更新し、比較表へ反映する
- 行政界未収録のため移動を2km以内へ制限し、都県境をまたぐ場合は住所検索を使う

完了条件: 色だけに依存せずピン位置と周辺の値を読め、意図しないドラッグで比較結果が変わらない。
...
1. 東京都内の住所を1件調べ、洪水とは別に、東京都・地震時の総合危険度と建物倒壊・火災・活動困難係数・地盤分類の内訳を見る。
...
デモ地点は、通常区域内、複数河川重複、境界付近をそれぞれ確実に再現できるfixtureから選ぶ。
